...
from selfplay_evaluator import AsyncEvaluator
...
def train_loop(process_idx, env, agent, steps, outdir, counter,
    ...
    try:
        ...
        while True:
            ...
            # a_t
            a, pout, opponent_pout = agent.act_and_train(obs, r)
            # o_{t+1}, r_{t+1}
            obs, r, done, info = env.step(a, pout, opponent_pout)
            local_t += 1
            episode_r += r
            episode_len += 1
...
def selfplay_train_agent_async(outdir, processes, make_env,
                      profile=False,
                      steps=8 * 10 ** 7,
                      eval_interval=10 ** 6,
                      eval_n_steps=None,
    ...
    def run_func(process_idx):
        ...
        env = make_env(process_idx, test=False, action_num=action_num)
        if evaluator is None:
            eval_env = env
        else:
            eval_env = make_env(process_idx, test=True, action_num=action_num)
