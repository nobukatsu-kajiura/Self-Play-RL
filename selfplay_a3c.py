...
class SelfPlay_A3C(agent.AttributeSavingMixin, agent.AsyncAgent):
    ...
    def __init__(self, model, optimizer, t_max, gamma, batchsize, action_num, beta=1e-2,
    ...
    def update(self, statevar):
        ...
        wins = np.zeros(self.action_num)
        loses = np.zeros(self.action_num)

        for n in range(self.batchsize):
            if self.past_rewards[self.t-1][n] == 1:
                for i in range(self.t_start, self.t):
                    wins[self.past_actions[i][n]] += 1
            else:
                for i in range(self.t_start, self.t):
                    loses[self.past_actions[i][n]] += 1
        ...
        for n in range(self.batchsize):
            ...
            for i in reversed(range(self.t_start, self.t)):
                ...
                R *= self.gamma
                R += self.past_rewards[i][n]
                if self.use_average_reward:
                    R -= self.average_reward
                v = self.past_values[i, n]
                advantage = R - v
                if self.use_average_reward:
                    self.average_reward += self.average_reward_tau * \
                        float(advantage.array)
                # Accumulate gradients of policy
                log_prob = self.past_action_log_prob[i, n]
                entropy = self.past_action_entropy[i, n]

                # Log probability is increased proportionally to advantage
                pi_loss_temp -= log_prob * float(advantage.array)
                # Entropy is maximized
                pi_loss_temp -= self.beta * entropy
                # Accumulate gradients of value function

                v_loss_temp += (v - R) ** 2 / 2
                
                if self.past_rewards[self.t-1][n] == 1:
                    if wins[self.past_actions[i][n]] == 0:
                        weight = 0
                    else:
                        weight = np.min(wins) / wins[self.past_actions[i][n]]
                else:
                    if loses[self.past_actions[i][n]] == 0:
                        weight = 0
                    else:
                        weight = np.min(loses) / loses[self.past_actions[i][n]]

                pi_loss += weight * pi_loss_temp
                v_loss += weight * v_loss_temp
        
        pi_loss /= self.batchsize
        v_loss /= self.batchsize
    ...
    def act_and_train(self, obs, reward):
        ...
        pout, vout, opponent_pout = self.model.pi_and_v(statevar)
        ...
        for i in range(self.batchsize):
            self.past_action_entropy[self.t, i] = pout.entropy[i]
            self.past_action_log_prob[self.t, i] = pout.log_prob(action)[i]
            self.past_values[self.t, i] = vout[i]
            # Update stats
            self.average_value += (
                (1 - self.average_value_decay) *
                (float(vout.array[i]) - self.average_value))
            self.average_entropy += (
                (1 - self.average_entropy_decay) *
                (float(pout.entropy.array[i]) - self.average_entropy))
        ...
        return action, pout, opponent_pout

    def act(self, obs):
        # Use the process-local model for acting
        with chainer.no_backprop_mode():
            statevar = self.batch_states([obs], np, self.phi)
            pout, _, opponent_pout = self.model.pi_and_v(statevar)
            if self.act_deterministically:
                return pout.most_probable.array, pout, opponent_pout
            else:
                return pout.sample().array, pout, opponent_pout
